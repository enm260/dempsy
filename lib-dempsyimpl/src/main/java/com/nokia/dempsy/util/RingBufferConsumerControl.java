...
/**
 * <p>This code is substantially based on the ingenious work done by Martin 
 * Thompson on what he calls "Mechanical Sympathy." It leans heavily on 
 * the source code from version 3.0.0.beta2 of the LMAX-exchange Disruptor
 * but has been completely refactored in order to invert separate the control
 * mechanism from what is being controlled and to simplify the API.</p>
 * 
 * <p>For more information on the LMAX Disruptor, see:</p>
 * 
 * <pre>     http://lmax-exchange.github.com/disruptor/</pre>
 * <hr>
 * <p>Employ Martin Thompson's "mechanical sympathy" to the concurrency control mechanism
 ...
 * their ThreeToOne test (on my machine, 64bit Linux, 64bit Java 6 Jvm).</p>
 * 
 * <p>This class is incredibly temperamental and must strictly be used the way it was
 * intended. Misuse can easily lead to lockups, missed sequences, etc.</p>
 * 
 * <p>In general, the {@link RingBufferControl} is the logic that controls the entries 
 * in a ring buffer, but not the ring buffer itself. These classes make no assumptions
 * about where the data is stored (other than in physical memory), or what type it is.</p>
 * 
 * <p>The {@link RingBufferControl} is completely analogous to a traditional "condition 
 ...
 * and consuming of data in a ring buffer.</p>
 * 
 * <p>The 'consumer side' control and the 'publishing side' control are broken into two
 * separate classes. This class represents control of the consumer side of the ring buffer.
 * while {@link RingBufferControl} additionally contains the publish side control.</p> 
 * 
 * <p>These two base primitives can only be used with one consuming thread and one
 * publishing thread, however, they form the building blocks for several other 
 * configurations (see {@link RingBufferControlMulticaster} and {@link RingBufferControlMultiplexor})</p>
 */
public abstract class RingBufferConsumerControl
{
    ...
    * {@link RingBufferControl#tryAvailableTo()} to inform the consumer that the
    * published has called {@link RingBufferControl#publishStop()}
    */
   public static final long ACQUIRE_STOP_REQUEST = -2L;
   
   /**
    * This value can be returned from {@link RingBufferControl#tryAvailableTo()}
    * to indicate that there are no pending published values available.
    */
   public static final long UNAVAILABLE = -1L;
   
   /**
    * This interface can be implemented to provide various custom wait strategies
    * for the consumer side of the control.
    */
   ...
   /**
    * Using this strategy provides 'spin' approach to waiting for data to 
    * be available for the consumer.
    */
   ...
   /**
    * Using this strategy provides 'yield' approach to waiting for data to 
    * be available for the consumer. It starts by spinning but quickly backs
    * off to yielding the thread in between polls.
    */
...
//   private final Object obj0 = null;
   ...
   protected final PaddedLong stop = new PaddedLong(Long.MAX_VALUE);
   ...
   /**
    * <p>{@link RingBufferControl#availableTo()} is a consumer side call that will block using the
    * wait strategy until a value (or several) have been published. It will return the sequence
    * that represents the position the publisher has published to.</p>
    * 
    * <p>This method can return {@link RingBufferControl#ACQUIRE_STOP_REQUEST} which means the 
    * publisher has called {@link RingBufferControl#publishStop()}. Once the consumer receives
    * this value the {@link RingBufferControl} has been reset.</p>
    */
   ...
   /**
    * <p>This method allows the consumer side to poll for publishing events. It will return 
    * what {@link RingBufferControl#availableTo()} will return but can also return
    ...
    */
   public boolean isShutdown() { return tail.get() == INITIAL_CURSOR_VALUE; }
   ...
   protected Sequence getTail() { return tail; }
